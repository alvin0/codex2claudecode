...
describe("runtime state line", () => {
  test("renders provider-aware starting states", () => {
    ...
    expect(runtimeLine({ status: "starting" }, "localhost", 8787, "copilot")).toBe("Starting Copilot runtime...")
  ...
  test("renders provider-aware running states", () => {
    ...
    expect(runtimeLine({ status: "running", server: {} as any, startedAt }, "localhost", 8787, "copilot")).toContain("Copilot runtime listening on http://localhost:8787")
  })
  ...
  test("renders mode-independent error state", () => {
    ...
    expect(runtimeLine({ status: "error", error: "port in use" }, "localhost", 8787, "copilot")).toBe("Runtime error: port in use")
  })
})
