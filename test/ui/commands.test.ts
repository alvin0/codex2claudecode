...
describe("UI commands", () => {
  ...
  test("returns Copilot commands in order", () => {
    expect(getCommands("copilot").map((command) => command.name)).toEqual([
      "/logs",
      "/connect",
      "/account",
      "/set-claude-env",
  ...
  test("uses dynamic switch-provider descriptions", () => {
    expect(getCommands("codex").find((command) => command.name === "/switch-provider")?.description).toBe("Switch upstream provider: Codex / Kiro / Copilot")
    expect(getCommands("kiro").find((command) => command.name === "/switch-provider")?.description).toBe("Switch upstream provider: Codex / Kiro / Copilot")
    expect(getCommands("copilot").find((command) => command.name === "/switch-provider")?.description).toBe("Switch upstream provider: Codex / Kiro / Copilot")
  })
  ...
  test("omits Codex-only commands in Copilot mode", () => {
    const names = getCommands("copilot").map((command) => command.name)
    expect(names).not.toContain("/codex-fast-mode")
    expect(names).toContain("/connect")
    expect(names).toContain("/account")
  })
