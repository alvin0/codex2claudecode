...
describe("runtime state properties", () => {
  test("running output contains provider label and URL", () => {
    fc.assert(
      fc.property(fc.constantFrom<ProviderMode>("codex", "kiro", "copilot"), fc.string({ minLength: 1 }), fc.integer({ min: 1, max: 65535 }), fc.integer({ min: 0 }), (mode, hostname, port, startedAt) => {
        const line = runtimeLine({ status: "running", server: {} as any, startedAt }, hostname, port, mode)
        expect(line).toContain(mode === "codex" ? "Codex" : mode === "kiro" ? "Kiro" : "Copilot")
  ...
  test("error output is mode-independent", () => {
    fc.assert(
      fc.property(fc.constantFrom<ProviderMode>("codex", "kiro", "copilot"), fc.string(), (mode, errorMessage) => {
