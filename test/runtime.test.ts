...
describe("runtime server", () => {
  ...
  test("Kiro runtime root advertises only supported endpoints", async () => {
    ...
    try {
      ...
      expect(body.endpoints.responses).toBeUndefined()
      expect(body.endpoints.chat_completions).toBeUndefined()
      ...
      expect((await originalFetch(`${base}/v1/responses`, { method: "POST", body: JSON.stringify({ model: "m", input: "hi" }) })).status).toBe(404)
      await expect(readFile(requestLogFilePath(kiroAuth), "utf8")).resolves.toContain('"/v1/responses"')
    } finally {
      server.stop(true)
    }
  })
